...
def choose_files():
    try:
        # Uzyskaj ścieżkę do folderu, w którym znajduje się plik wykonywalny .exe
        work_folder = os.path.dirname(sys.executable)

        # Ustaw folder roboczy na odpowiednią ścieżkę
...
def generate_plots_from_excel(excel_file):
    # Uzyskaj ścieżkę do folderu, w którym znajduje się plik wykonywalny .exe
    work_folder_w = os.path.dirname(sys.executable)
    # Ustaw folder roboczy na odpowiednią ścieżkę
    os.chdir(work_folder_w)
    ...
    plt.legend(labels=legend_labels, loc="upper center", bbox_to_anchor=(0.5, -0.1), title='Zakres SPF', ncol=6)
